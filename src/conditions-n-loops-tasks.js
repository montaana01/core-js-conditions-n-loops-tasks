...
function convertToRomanNumerals(num) {
  ...
  for (let i = 0; i < NUMBER_TO_NUMERIALS.length; i += 1) {
    while (number >= NUMBER_TO_NUMERIALS[i].value) {
      result += NUMBER_TO_NUMERIALS[i].numeral;
      number -= NUMBER_TO_NUMERIALS[i].value;
    }
  }
  return result;
}
  ...
  function quickSort(sortArray, low, high) {
    if (low < high) {
      ...
      for (let j = low; j <= high - 1; j += 1) {
        if (arr[j] < pivot) {
          pi += 1;
          [newArr[pi], newArr[j]] = [newArr[j], newArr[pi]];
        }
      }
      [newArr[pi + 1], newArr[high]] = [newArr[high], newArr[pi + 1]];
      pi += 1;
...
function shuffleChar(str, iterations) {
  ...
  for (let iteration = 0; iteration < ITERATION; iteration += 1) {
    let evenChars = '';
    let oddChars = '';
...
function getNearestBigger(number) {
  ...
  const numberArray = [];
  while (startNumber > 0) {
    numberArray.unshift(startNumber % 10);
    startNumber = Math.floor(startNumber / 10);
  }

  let i = numberArray.length - 2;
  while (i >= 0 && numberArray[i] >= numberArray[i + 1]) {
    i -= 1;
  }

  if (i < 0) {
    return numberArray.reduce((acc, digit) => acc * 10 + digit, 0);
  }

  let j = numberArray.length - 1;
  while (numberArray[j] <= numberArray[i]) {
    j -= 1;
  }

  [numberArray[i], numberArray[j]] = [numberArray[j], numberArray[i]];

  for (let k = i + 1, l = numberArray.length - 1; k < l; k += 1, l -= 1) {
    [numberArray[k], numberArray[l]] = [numberArray[l], numberArray[k]];
  }
  return numberArray.reduce((acc, digit) => acc * 10 + digit, 0);
}
